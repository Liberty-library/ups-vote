...
function new_post($s, $title, $text = null, $image_location = null) {
    if (!$title) {
    ...
    return tquery("INSERT INTO posts (user_id, soc_id, title, text, image_location, thumbnail_location)
                   VALUES (?, ?, ?, ?, ?, ?)", [
        $_SESSION["user"]["user_id"],
        $s["soc_id"],
        $title,
...
function insertGuestID($pdo, $guestID) {
    $stmt = $pdo->prepare("INSERT INTO guest (guest_id) 
                          SELECT :guestID 
                          WHERE NOT EXISTS (SELECT 1 FROM guest WHERE guest_id = :guestID)");
    $stmt->bindParam(':guestID', $guestID, PDO::PARAM_STR); // Specify the data type as string
    $stmt->execute();
}
?>
