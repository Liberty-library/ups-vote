<?php
   ...
   /* function log_soc_activity($sid)
	 ...
	 */
	function verify_access($allowGuest = false)
{
    session_start();
    
    if (!$allowGuest && !isset($_SESSION["user"]))
    {
        redirect("login.php");
    }
}
	...
	function generate_thumbnail($source_path, $destination_path, $max_width = 150, $max_height = 150) {
    ...
    imagejpeg($thumbnail, $destination_path, 80); // Save as JPG with 80% quality
...
function generate_video_thumbnail($video_path, $thumbnail_path) {
    $command = "ffmpeg -i $video_path -ss 00:00:01 -vframes 1 $thumbnail_path";
    exec($command);
}
?>
