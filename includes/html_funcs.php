<?php
	...
	*/
	...
	{
	if (!is_array($a)) {
    var_dump($a); // Show the unexpected value
    debug_print_backtrace(); // Show where it came from
    die();
}
		$html = "<".$a["tag"];
		// opening tag
...
{
    // Thumbnail for video or post (use a default if none exists)
    $thumbnail_url = $p["thumbnail_url"] ? $p["thumbnail_url"] : "uploads/style/no_thumb_loaded.PNG";
    $thumbnail_html = "<div class='video-thumbnail-container'>
                        <img src='" . htmlspecialchars($thumbnail_url, ENT_QUOTES, 'UTF-8') . "' 
                             class='img-thumbnail' 
    ...
    // Main post div (vote buttons and content)
    $div = div(post_vote_buttons($p), "list-group-item container-fluid post-summary".(($p["status"] == "STICKIED") ? " post-stickied" : ""));

    // Add thumbnail and content side by side
    $div["children"][] = div([$thumbnail_html, $div2], "row");
	...
	function post_summary1($p, $sname, $show_soc = false)
	{
		$a = a("post.php?pid=".$p["post_id"]."&soc=".$sname);
		$title = div(h(4, $p["title"]."\t(".(($p["votes"]>0) ? "+":"").$p["votes"].")", "list-group-item-heading post-title", "post-title-".$p["post_id"]));
		if ($p["status"]=="STICKIED")
	...
	*/
	...
	function soc($sname)	{ return to_html(soc_link($sname)); }
	       ...
	       function post_thumb($p)
{
    if (isset($p["image_location"]) && is_string($p["image_location"]) && file_exists($p["image_location"])) {
        if (strtolower(pathinfo($p['image_location'], PATHINFO_EXTENSION)) == "mov" || strtolower(pathinfo($p['image_location'], PATHINFO_EXTENSION)) == "mp4") {
            echo '<video width="70" height="90" controls muted>
                <source src="' . $p['image_location'] . '" type="video/mp4">
            </video>';
        } else {
            echo '<div class="container-fluid-thumb">';
            echo '<img src="' . $p['image_location'] . '" class="img-thumbnail" data-action="expand"/>';
            echo '</div>';
        }
    } else {
        echo '<div class="container-fluid-thumb">';
        echo '<img src="uploads/style/no_thumb_loaded.PNG" class="img-thumbnail"/>';
        echo '</div>';
    }
}
?>
