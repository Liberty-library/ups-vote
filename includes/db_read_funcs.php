...
function get_socc($sname)
{
    $retval = ($rows = query("SELECT * FROM soc_cover WHERE soc_cover_location = ?", $sname));
...
function get_society_css($soc_id)
{
    // Assuming your database connection and query functions are appropriately defined

    // Fetch CSS from the 'soc_css' table based on soc_id and user_id
    $result = query("SELECT css_text FROM soc_css WHERE soc_id = ? ", $soc_id);

    if ($result !== false && sizeof($result) > 0) {
        return $result[0]['css_text'];
    }

    return ''; // Return an empty string if no CSS is found
    
    $result = $conn->query($query);

// Fetch the result
$customCSS = $result->fetch_assoc();

// Print the contents of $customCSS
print_r($customCSS);
}
...
d.revised_by, 
d.info, 
DATE_FORMAT(d.time, '%H:%i, %b %d, %Y') AS 'time',
r.username AS revised_by,
c.username AS c_name,
sc1.soc_cover_location  -- Include soc_cover_location from first join
FROM societies s 
JOIN users c ON c.user_id = s.created_by
LEFT JOIN soc_details d ON s.soc_id = d.soc_id
JOIN users r ON r.user_id = d.revised_by
LEFT JOIN soc_cover sc1 ON s.soc_name = sc1.soc_id  -- First JOIN with alias sc1
LEFT JOIN soc_cover sc2 ON s.image_location2 = sc2.soc_cover_location -- Second JOIN with alias sc2
WHERE s.soc_name = ?",
                            $sname
                        )
            )
	...
	function get_posts($soc, $offset = 0, $lim = 20)
{
    return query("	SELECT  p.*,
								u.username, 
								IF(p.status='STICKIED',10,0) AS `rank`,
								COALESCE(SUM(IF(v.vote='UP',1,IF(v.vote='DOWN',-1,0))), 0) AS votes,
								(SELECT COUNT(*) 
								 FROM post_views w
								 WHERE w.post_id = p.post_id
								) AS views,
	...
	function get_article_feed($lim = 100, $offset = 0)
{
    // Corrected SQL query
    $sql = "SELECT a.* 
            FROM articles a 
    ...
    $result = query($sql, $lim, $offset);

    return $result;
}
	
	function get_all_posts($offset = 0, $lim = 20)
{
    return query("SELECT  p.*,
                        u.username, 
                        if(p.status='STICKIED', 10, 0) as \"rank\",
...
function get_post($pid)
{
    return ($p = query("
        SELECT  p.*, 
                u.username,
                sum(if(v.vote='UP',1,if(v.vote='DOWN',-1,0))) as votes,
                (SELECT count(*) FROM post_views w WHERE w.post_id = p.post_id) as \"views\",
                ...
                p.image_location as image_url  -- Use the image_location column
        FROM posts p 
...
{
    // Modify the SQL query to not rely on user-related data
    $sql = "SELECT  p.*,
                    u.username, 
                    s.soc_name \"society\",
                    sum(if(v.vote='UP',1,if(v.vote='DOWN',-1,0))) as votes,
            ...
            FROM posts p 
            JOIN (SELECT soc_id FROM soc_subs) ss on p.soc_id = ss.soc_id
...
function get_news_sort($lim = 100, $offset = 0, $sort_by = 'votes')
{
    // Ensure compatibility with PHP 7+
    switch ($sort_by) {
        case 'date':
            $order_by = 'p.time DESC'; // Sort by newest posts first
            break;
        case 'votes':
        default:
            $order_by = 'votes DESC, comments DESC, views DESC'; // Default sorting
            break;
    }
    ...
    $sql = "SELECT  
            ...
            JOIN users u ON p.user_id = u.user_id
            WHERE p.status != 'DELETED'
            GROUP BY p.post_id, u.username, s.soc_name
            ORDER BY $order_by
            LIMIT ? OFFSET ?";

    return query($sql, $lim, $offset); // Adjust based on your query function's parameter style
}
